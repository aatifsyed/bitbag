...
#[proc_macro_derive(BitBag)]
pub fn derive(input: TokenStream) -> TokenStream {
    pretty_env_logger::init();
    let user_struct = parse_macro_input!(input as DeriveInput);
    debug!("{:#?}", user_struct);

    if let syn::Data::Enum(ref user_enum) = user_struct.data {
        // Pull out the `u8` in `#[repr(u8)]`
        if let Some(primitive) = user_struct.attrs.iter().find_map(|attribute| {
            ...
            let generated_ident =
                Ident::new(&format!("{}Bag", user_struct.ident), user_struct.span());
            let appended = quote! {
                struct #generated_ident(#primitive);
            };
            debug!("{:#?}", appended);
            appended.into()
        } else {
